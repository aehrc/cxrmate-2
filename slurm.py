...
class SlurmSubmit(object):
    """
    Submits a job to Slurm, a cluster manager.
    """
    def __init__(
        self,
        args: Namespace,
        ...
        Argument/s:
            ...
            manager_script_path - path to the manager script.
            fnc - function for the job.
            is_main_process - flag for main process.
            ...
            exit_after_submit - flag for exiting after submitting job.
            run_cmd - cluster manage run command.
            job_type - add a descriptor of the job type.
        """
        self.args = args
        self.task = task
        self.config = config
        ...
        self.manager_script_path = manager_script_path
        self.fnc = fnc
        ...
        self.exit_after_submit = exit_after_submit
    ...
    def submit(self, job_display_name=None):
        result = None
        self.job_display_name = job_display_name + '_' + f'{self.job_type}_' if self.job_type else job_display_name + '_'
        ...
        if self.is_from_manager_object:
            ...
            timestamp = f'session_{session}_{self.job_type}_{timestamp}' if self.job_type else f'session_{session}_{timestamp}'
            ...
            print(f'\nLaunching experiment {self.run_cmd} {manager_script_path}...')
            result = call(f'{self.run_cmd} {manager_script_path}', shell=True)
            assert result == 0, f'Error launching experiment: {manager_script_path}.'
            print(f'Launched experiment {manager_script_path}.')

        if self.exit_after_submit:
    ...
    def create_slurm_script(self, manager_script_path, timestamp, session):
        ...
        if self.no_ntasks_per_node:
            script.append(f'#SBATCH --ntasks-per-node={self.no_ntasks_per_node}')
        ...
        if self.auto_resubmit_method == 'signal':
            script.append(f'#SBATCH --signal=USR2@{5 * 60}')
        ...
        cmd_line_args = []
        cmd_line_args.append(f'--task {self.task}')
        cmd_line_args.append(f'--config {self.config}')
        cmd_line_args.append(f'--trial {self.trial}')
        cmd_line_args.append(f'--exp_trial_dir {self.save_dir}')
        ...
        if self.auto_resubmit_method == 'timeout':
            ...
            script.append(f'    {self.run_cmd} {manager_script_path}')
